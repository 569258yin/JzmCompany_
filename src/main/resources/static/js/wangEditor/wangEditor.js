    ...
    E.createMenu(function (check) {
        ...
        $content.on('click', 'a[commandValue]', function (e) {
            ...
            if (menu.selected && editor.isRangeEmpty()) {
                // 当前处于选中状态，并且选中内容为空
                editor.commandForElem('fonts[color]', e, 'forecolor', commandValue);
            } else {
                // 当前未处于选中状态，或者有选中内容。则执行默认命令
        ...
        menu.updateSelectedEvent = function () {
            var rangeElem = editor.getRangeElem();
            rangeElem = editor.getSelfOrParentByName(rangeElem, 'fonts[color]');
            if (rangeElem) {
                return true;
            }
            return false;
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        menu.updateSelectedEvent = function () {
            var rangeElem = editor.getRangeElem();
            rangeElem = editor.getSelfOrParentByName(rangeElem, 'span,fonts', checkElemFn);
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        */
        ...
        // 创建droplist
        var tpl = '<span style="fonts-family:{#commandValue};">{#title}</span>';
        menu.dropList = new E.DropList(editor, menu, {
            data: data,
            tpl: tpl,
            selectorForELemCommand: 'fonts[face]'  // 为了执行 editor.commandForElem 而传入的elem查询方式
        });

        // 定义 update selected 事件
        menu.updateSelectedEvent = function () {
            var rangeElem = editor.getRangeElem();
            rangeElem = editor.getSelfOrParentByName(rangeElem, 'fonts[face]');
            if (rangeElem) {
                return true;
            }
            return false;
...
_e(function (E, $) {
    E.createMenu(function (check) {
        ...
        */
        ...
        // 创建droplist
        var tpl = '<span style="fonts-size:{#title};">{#title}</span>';
        menu.dropList = new E.DropList(editor, menu, {
            data: data,
            tpl: tpl,
            selectorForELemCommand: 'fonts[size]'  // 为了执行 editor.commandForElem 而传入的elem查询方式
        });

        // 定义 update selected 事件
        menu.updateSelectedEvent = function () {
            var rangeElem = editor.getRangeElem();
            rangeElem = editor.getSelfOrParentByName(rangeElem, 'fonts[size]');
            if (rangeElem) {
                return true;
            }
            return false;
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        // dropPanel 内容
        var $content = $('<div style="fonts-size: 14px; color: #666; text-align:right;"></div>');
